...
class TestWebSocketDeletePlayerBroadcast:
    def test_delete_player_broadcasts_scoreboard(self, app, client, admin_client, ws_client):
        ...
        start_game(admin_client)
        register_player(client, make_player_id("player-ws-del"), "WSDelPlayer")
        admin_client.post(f"/admin/api/players/{make_player_id('player-ws-del')}/adjust", json={"delta": 50})

        ws_client.get_received()

        admin_client.delete(f"/admin/api/players/{make_player_id('player-ws-del')}")
    ...
    def test_bulk_delete_players_broadcasts_scoreboard(self, app, client, admin_client, ws_client):
        ...
        start_game(admin_client)
        register_player(client, make_player_id("player-ws-bulk"), "WSBulkPlayer")
